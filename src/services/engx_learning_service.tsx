...
export default class EngXLearningService {
    ...
    public getWordPronunciation(word: string) {
        var pronunciation = this.question_mapper[this.language].pronunciation
            .replace('{0}', this.age_mapper[this.language][this.age])
            .replace('{1}', word)
        return this.gpt_service.prompt(pronunciation)
            .then(messages => messages?.shift())
    }

    public getWordDefinition(word: string) {
        var definition = this.question_mapper[this.language].definition
            .replace('{0}', this.age_mapper[this.language][this.age])
            .replace('{1}', word)
        return this.gpt_service.prompt(definition)
            .then(messages => messages?.shift())
    }

    public getWordExample(word: string) {
        var example = this.question_mapper[this.language].example
            .replace('{0}', this.age_mapper[this.language][this.age])
            .replace('{1}', word)
        return this.gpt_service.prompt(example)
            .then(messages => messages?.shift())
    }
}