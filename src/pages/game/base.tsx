...
//           .getGameOfWords(["Hello World", "Encapsulation", "Inheritance", "Polymorphism", "Abstraction"])
//           .then((quiz) => {
//             setQuiz(quiz);
//             setQid(0);
...
//           </div>
//         </div>
//         <div className="bg-white p-16 rounded-lg shadow-lg flex flex-col gap-10 h-full overflow-y-auto">
//           {quiz == null ? (
...
export default function BaseGame() {
  ...
  addEventListener("resize", () => {
    gameStore.setWidth(window.innerWidth / 1.2);
    gameStore.setHeight(window.innerHeight / 1.4);
  });
  ...
  return (
    <>
      <Stage
        className="border-5 rounded-xl border-slate-500 shadow-xl"
        style={{
          fontFamily: "VT323",
        }}
      ...
      >
        {gameStore.isLoading ? <LoadingScene /> : <CombatScene />}
      </Stage>
    </>
  );
}
