      ...
      <div className="pt-32 py-16">
        <div className="bg-white p-16 px-16 rounded-lg shadow-lg h-fit justify-between text-6xl font-bold text-red-500 flex flex-col gap-10">
          ...
          <div className="flex justify-between">
            <Button className="w-1/3 font-bold" color="primary" onClick={() => navigate(0)}>
              Try again
            </Button>
            <Button className="w-1/3 font-bold" color="danger" onClick={() => navigate("/home")}>
              Exit
            </Button>
          </div>
        </div>
        ...
        <div className="bg-white p-5 px-16 rounded-lg shadow-lg flex h-1/5 justify-between">
          {hp != -1 ? (
            <div className="flex gap-5 items-center justify-center h-full">
              <div className="text-red-500 text-xl font-bold align-middle items-center">HP: {hp}</div>
              <div className="h-full flex items-center">
                {Array.from(Array(hp).keys()).map(e => (
                  <div className="h-5 w-8 bg-red-500 rounded border-2 border-red-200"></div>
                ))}
              </div>
            </div>
          ) : (
          ...
          <div className="flex items-center font-bold text-xl">Score: {score}</div>
        </div>
        <div className="bg-white p-16 rounded-lg shadow-lg flex flex-col gap-10 h-full">
          {quiz == null ? (
            <div className="flex items-center justify-center font-bold text-6xl h-full">Loading</div>
          ) : (
                  ...
                  {!!index && (
                    ...
                    >
                      {index - 1 < qid
                        ...
                        : "_".repeat(quiz?.questions[index - 1]._correct_answer_str.length)}
                    </span>
                  )}
                  {paragraph}
                </React.Fragment>
        ...
        <div className="bg-white p-5 rounded-lg shadow-lg flex gap-5 justify-evenly h-1/4">
          {qid == -1 ? (
            ...
            quiz?.questions[qid].answers.map((answer, id) => (
              <Button
                color={viewAnswer ? (id == quiz!.questions[qid].correct_answer ? "success" : "danger") : "primary"}
